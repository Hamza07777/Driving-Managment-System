...
<div class="box">
   <div class="box-header">
      ...
      @endif
      <!--<a href="{{ route('student_package.create') }}"  class="btn btn-primary pull-right "><i class="fa fa-plus-circle" style="margin-right: 8%;"></i>{{ __('trans.Add_Student_Package') }} </a>-->
      ...
      <i class="fa fa-plus-circle" style="margin-right: 8%;"></i>{{ __('trans.Add_Student_Package')}}
      </button>
      <h5>{{ __('trans.Student_Packages') }} </h5>
   </div>
   <!-- /.box-header -->
      ...
      <table id="advance-1" class="display">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td scope="row">{{!empty($package->student_package_package->detail) ?  $package->student_package_package->detail:''}}</td>
               @if(!empty($package->student_package_student->first_name) && !empty($package->student_package_student->last_name))         
               <td scope="row">{{$package->student_package_student->first_name}} {{$package->student_package_student->last_name}}</td>
               @else
               <td>  </td>
               @endif
               <td scope="row">
               ...
               <td scope="row" >
                  <!--<a href="{{url('student_package/'.$package->id.'/edit')}}" style=""><i class="fa fa-edit" style="font-size: 25px; margin-left: 10%;"></i></a>-->
                  <button class="btn" style="background: transparent;"  onclick="edit_student_package({{$package->id}})" style=""><i class="fa fa-edit" style="font-size: 25px; margin-left: 10%;"></i></button>
                  <form action="{{url('student_package',$package->id)}}" method="POST" style="float: right ;margin-right: 32%;margin-top: 3%;">
                     @csrf
                     @method('DELETE')
      ...
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            <div class="modal-header">
               <h5 class="modal-title" id="exampleModalLabel">{{ __('trans.Add_Student_Package') }}</h5>
               <button type="button" class="close" data-dismiss="modal" aria-label="Close">
               <span aria-hidden="true">&times;</span>
               </button>
            </div>
            <div class="modal-body">
               <form method="POST" action="{{ route('student_package.store') }}" enctype="multipart/form-data" >
                  @csrf
                  ...
                  <div class="form-group">
                     <label for="package_id" class="col-form-label text-md-right">{{ __('trans.Package_Name') }}</label>
...
<div class="modal right fade" id="myModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel3" aria-hidden="true">
   <div class="modal-dialog" role="document">
      <div class="modal-content">
         <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">{{ __('trans.Update_Student_Package') }}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
         </div>
         <div class="modal-body">
            <form method="POST" action="/student_package/" enctype="multipart/form-data" id="branchform">
               @csrf
               {{ method_field('PUT') }}
               <div class="form-group">
                  <label for="branch_id" class=" col-form-label text-md-right">{{ __('trans.Brach_Name') }}</label>
   ...
   <script>
      ...
      //           return confirm('Are Your Sure to Unsubscribe Student From Package, Data Related To This Will Also  Delete. Data Will Not Recoverd ?');
      //       }
          ...
          var form = $(this).parent();
            ...
            theme: 'dark',
            title: 'Delete',
            content: 'Are Your Sure To Unsubscribe Student From Package, Data Related To This Will Also Delete. Data Will Not Recoverd ?',
           buttons: {
              Delete: function () {
                  form.submit();
              },
      ...
      function all_packages(id){
      
          $.ajax(
          {
              url: "{{URL::to('all_package_data')}}",
              type: 'GET',
              dataType: 'json',
               data: {
                  "id": id,
      ...
      function all_packages_branch(id){
              ...
              success: function (response){
                 ...
                 if(len > 0){
                      for(var i=0; i<len; i++){
                          ...
                          var name = response['data'][i].detail;
                           var tr_str = '<option value="' +id+ '">' + name + '</option>'
                          $("#branchform #package_id").append(tr_str);
                      }
                 }
      ...
      function edit_student_package(id){
       ...
       <?php 
         $branch = DB::table('branches')->get(); 
          ...
          $.ajax(
          {
              ...
              //   }
                          var package_id = response['data'].package_id;
              ...
              var tr_str = '<option value="'+package_id+'" selected>'+detail+'</option>';
      ...
      function checkFluency(id){
          ...
          $.ajax(
          {
              url: "student_package_change_status/"+id,
              type: 'GET',
               dataType: 'json',
              // data: {
              //     "id": id,
